...
const ExtensionApp: React.FC = () => {
  ...
  const loadGuestSettings = async (): Promise<AppSettings> => {
    const keys = [
      "theme", "mainFont", "codeFont", "fontSize", 
      "animationsDisabled", "customInstruction", "hidePersonalInfo", "disableLinkWarning"
    ];
    
    const values = await extensionStorage.getMultiple(keys);
    
    return {
      theme: (values.theme as Theme) || "system",
      mainFont: values.mainFont || "Montserrat",
      codeFont: values.codeFont || "JetBrains Mono (default)",
      fontSize: parseFloat(values.fontSize || "1"),
      animationsDisabled: values.animationsDisabled === "true",
      customInstruction: values.customInstruction || "",
      hidePersonalInfo: values.hidePersonalInfo === "true",
      disableLinkWarning: values.disableLinkWarning === "true",
    };
  };
  ...
  const [settings, setSettings] = useState<AppSettings>({
    theme: "system",
    mainFont: "Montserrat",
    codeFont: "JetBrains Mono (default)",
    fontSize: 1,
    animationsDisabled: false,
    customInstruction: "",
    hidePersonalInfo: false,
    disableLinkWarning: false,
  });
  ...
  const clearLocalSettings = async () => {
    ...
    for (const key of guestSettingsKeys) {
  ...
  const updateSettings = async (newSettings: Partial<AppSettings>) => {
    ...
    if (auth.currentUser) {
      ...
      await extensionStorage.setMultiple(newSettings);
    }
  };

  useEffect(() => {
    // Initialize settings from extension storage
    loadGuestSettings().then(initialSettings => {
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        const localSettings = await loadGuestSettings();
        const guestSettingsKeys: Array<keyof AppSettings> = [
          "theme",
          "mainFont",
          "codeFont",
        ...
        const hasLocalSettings = Object.values(await extensionStorage.getMultiple(guestSettingsKeys)).some(value => value !== null);
        ...
        try {
          ...
          if (docSnap.exists() && hasLocalSettings) {
            ...
            setSettings(await loadGuestSettings());
            setSettingsLoaded(true);
            setupFirestoreListener();
          }
        } catch (error) {
  ...
  useEffect(() => {
    console.log("Firebase Initialized in Extension:", app.name);
  }, []);
  ...
  return (
    <>
      {/* Extension-specific layout with optimized dimensions */}
      <div className="flex h-full w-full bg-white dark:bg-[#1c1c1c] text-zinc-900 dark:text-zinc-200 font-sans overflow-hidden">
        <Sidebar
          ref={sidebarRef}
